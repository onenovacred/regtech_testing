            ...
            <div class="card">
                <div class="card-primary">
                    <div class="card-header">
                        <h3 class="card-title">@if ($renew ?? '' == 'true') Renew Plan @else Edit User @endif</h3>
                    </div>
                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="rdo" value="role_postpaid" @if($user->type == 'role_postpaid') checked @endif>
                                            <label class="form-check-label">Postpaid</label>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="rdo" value="role_prepaid" @if($user->type == 'role_prepaid') checked @endif>
                    ...
                    <div class="row">
                                    ...
                                    <div class="col-md-3"></div>
                                    <div class="col-md-3" id="plan_duration_days">Plan Duration ({{$user_scheme_details[0]['duration']}} Days)</div>
                                ...
                                <div class="row text-center">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <select name="plan" id="plan" class="form-control selectpicker" data-live-search="true" data-actions-box="true">
                                            ...
                                            <option value="basic" @if($user_scheme_details[0]['plan'] == 'basic') selected @endif>Basic Plan</option>		
                                            <option value="starter"@if($user_scheme_details[0]['plan'] == 'starter') selected @endif>Starter Plan</option>		
                                            <option value="standard"@if($user_scheme_details[0]['plan'] == 'standard') selected @endif>Standard Plan</option>		
                                            <option value="advance"@if($user_scheme_details[0]['plan'] == 'advance') selected @endif>Advance Plan</option>		
                                            <option value="growth"@if($user_scheme_details[0]['plan'] == 'growth') selected @endif>Growth Plan</option>		
                                            <option value="unicorn"@if($user_scheme_details[0]['plan'] == 'unicorn') selected @endif>Unicorn Plan</option>		
                                        </select>
                                    </div>
                                    <div class="col-md-3">  
                                        <?php
                                        ...
                                        ?>
                                            <input class="form-control" type="text" name="plan_duration" id="plan_duration" value="{{$newStartDate}} to {{$newEndDate}}" placeholder="Plan Duration" disabled>
                                        <?php }else{ ?>
                                    ...
                                    <div class="col-md-3" style="padding-right: 15px;">
                                        <input class="form-control" type="text" name="plan_amount" id="plan_amount" value="{{$user_scheme_details[0]['plan_amount']}}" placeholder="Plan Amount" disabled>
                                    </div>   
                                </div>
                                <hr> 
                        <?php
                            $apis=DB::table('api_master')->select('id','api_name', 'admin_price','api_group_id')->get();
                        ?>
                        ...
                        <div class="form-group" id="demo">
                                @foreach($api_group as $key=>$value)
                                    ...
                                    <?php
                                        $sub_menus=DB::table('api_master')->select('id','table_name','api_name', 'admin_price','api_group_id')->where('api_group_id',$value->id)->get();
                                    ...
                                    @if(count($sub_menus)>0)
                                        @foreach ($sub_menus as $sub_menu)
                                            <div class="row" style="padding-bottom: 5px;">
                                                <div class="col-md-4">
                                                    <div class="custom-control custom-checkbox" style="margin-left: 8px;">
                                                        <input class="custom-control-input checkbox groupcheckboxdemo_{{$sub_menu->api_group_id}}" type="checkbox" id="chkdemo_{{$sub_menu->id}}_{{$sub_menu->api_group_id}}" name="chk_api_iddemo" value="{{$sub_menu->id}}" @if(in_array($sub_menu->id, $user_scheme_arr)) checked @endif>
                                                ...
                                                <div class="col-md-8">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-3">
                                                            <input type="text" class="form-control grouptextdemo_{{$sub_menu->api_group_id}}" id="txtUserPricedemo{{$sub_menu->id}}"  @if(!in_array($sub_menu->id, $user_scheme_arr)) disabled @else value="{{$user_scheme_price[$sub_menu->id]}}" @endif>
                                                        </div>
                                                        <div class="col-md-3">
                                                        <?php
                                                        $sub_menusid = DB::table('api_master')->select('table_name')->where('id', $sub_menu->id)->get();
                                ...
                                @foreach($api_group as $key=>$value)
                                    ...
                                    @if(count($sub_menus)>0)
                                        @foreach ($sub_menus as $key1=>$sub_menu)
                                            <div class="row" id="editContent{{$sub_menu->id}}" style="padding-bottom: 5px;">
                                                <div class="col-md-5">
                                                    <div class="custom-control custom-checkbox" style="margin-left: 8px;">
                                                        <div class="row">
                                                            <div class="col-md-5">
                                                            ...
                                                            <div class="col-md-7">
                                                                <select name="planType" id="planType_{{$sub_menu->id}}" class="planType form-control selectpicker" data-live-search="true" data-actions-box="true">
                                                                    ...
                                                                    <option value="regtech_plan" @if(isset($user_scheme_details[$key]) && isset($user_scheme_range[$sub_menu->id]) && $user_scheme_range[$sub_menu->id] == 'null') selected @endif>Regtech API Plan</option>		
                                                                    <option value="custom_plan" @if(isset($user_scheme_details[$key]) && isset($user_scheme_range[$sub_menu->id]) && $user_scheme_range[$sub_menu->id] != 'null') selected @endif>Custom Plan</option>		
                                                                </select>
                                                            </div>
                                                        </div>    
                                                    </div>
                                                ...
                                                @if(isset($user_scheme_details[$key]) && isset($user_scheme_range[$sub_menu->id]) && $user_scheme_range[$sub_menu->id] == 'null')
                                                    <input type="hidden" name="regtechplan{{$sub_menu->id}}" id="regtechplan{{$sub_menu->id}}" value="regtech_plan">
                                                @elseif(isset($user_scheme_details[$key]) && isset($user_scheme_range[$sub_menu->id]) && $user_scheme_range[$sub_menu->id] != 'null')
                                                    <input type="hidden" name="regtechplan{{$sub_menu->id}}" id="regtechplan{{$sub_menu->id}}" value="custom_plan">
                                                @else
                                                    <input type="hidden" name="regtechplan{{$sub_menu->id}}" id="regtechplan{{$sub_menu->id}}" value=""> 
                                                @endif       
                                                <div class="col-md-2 text-center" id="regtech_plan{{$sub_menu->id}}">
                                                    ...
                                                    ?>
                                                    <input type="text" class="form-control" id="txtPlanPrice{{$sub_menu->id}}" name="txtPlanPrice{{$sub_menu->id}}" value="{{$sub_menu->$scheme_price}}" disabled>
                                                    <?php 
                                                        }else{
                                                    ?>        
                                                ...
                                                <div class="col-md-7" id="custom_plan{{$sub_menu->id}}">
                                                    ...
                                                    <div class="field_wrapper{{$sub_menu->id}}">
                                                        <div class="row">
                                                            <div class="col-md-4">
                                                                <input type="text" class="form-control customRange_from{{$sub_menu->id}}" id="customRange_1_from{{$sub_menu->id}}" name="customRange_1_from{{$sub_menu->id}}" value="0" placeholder="hits range from...">
                                                            </div>
                                                            <div class="col-md-4">
                                                                <input type="text" class="form-control customRange_to{{$sub_menu->id}}" id="customRange_1_to{{$sub_menu->id}}" name="customRange_1_to{{$sub_menu->id}}" @if(isset($schemeRangeExploded[0])) value="{{$schemeRangeExploded[0]}}" @else value = "" @endif placeholder="hits range upto...">
                                                            </div>
                                                            <div class="col-md-2">
                                                                <input type="text" class="form-control" id="customPlanPrice1{{$sub_menu->id}}" name="customPlanPrice1{{$sub_menu->id}}" @if(isset($schemePriceExploded[0])) value="{{$schemePriceExploded[0]}}" @else value = "" @endif  placeholder="Rs.">
                                                            </div>
                                                            <div class="col-md-2" style="padding-top:5px;">
                                                        ...
                                                        if(count($schemePriceExploded) > 1){
                                                            foreach($schemePriceExploded as $key=>$price){
                                                                if(isset($schemeRangeExploded[$key+1])){ ?>
                                                                    <div id="rangeDiv{{$sub_menu->id}}{{$key+2}}" class="row" style="padding: 5px 0px">
                                                                        <div class="col-md-4">
                                                                            <input type="text" class="form-control customRange_from{{$sub_menu->id}}" id="customRange_{{$key+2}}_from{{$sub_menu->id}}" name="customRange_{{$key+2}}_from{{$sub_menu->id}}" value="{{$schemeRangeExploded[$key]}}" placeholder="hits range from...">
                                                                        </div>
                                                                        <div class="col-md-4">
                                                                            <input type="text" class="form-control customRange_to{{$sub_menu->id}}" id="customRange_{{$key+2}}_to{{$sub_menu->id}}" name="customRange_{{$key+2}}_to{{$sub_menu->id}}" value="{{$schemeRangeExploded[$key+1]}}" placeholder="hits range upto...">
                                                                        </div>
                                                                        <div class="col-md-2">
                                                                            <input type="text" class="form-control" id="customPlanPrice{{$key+2}}{{$sub_menu->id}}" name="customPlanPrice{{$key+2}}{{$sub_menu->id}}" value="{{$schemePriceExploded[$key+1]}}"  placeholder="Rs.">
                                                                        </div>
                                                                        <div class="col-md-2" style="padding-top:5px">
                                                                            <a href="javascript:void(0);" class="remove_button{{$sub_menu->id}}" onclick="removeRange({{$sub_menu->id}}{{$key+2}})"><i class="fa fa-fw fa-minus-circle"></i></a>
                                                                        </div>
                                                                    </div>
                                                                <?php 
                                                                    $priceRange = $key + 1;
                                                    ...
                                                    ?>        
                                                        <input type="hidden" id="priceRange{{$sub_menu->id}}" value="{{$priceRange}}" name="priceRange{{$sub_menu->id}}" />
                                                </div>
                        ...
                        <div class="text-center">
                            <button type="button" id="btnUpdate" class="btn btn-primary" style="width:40%; margin: 20px 0 20px 0">Update</button>
                        </div>
                        </form>
                    </div>
                </div>
