    ...
    <div class="col-md-6 offset-md-3">
        ...
        <div class="card card-primary">
            <div class="card-header">
                <h3 class="card-title">UAN Basic PAN v4</h3>
            </div>
            <div class="card-body">
                ...
                <form method="post" action="{{ route('kyc.upi_basic_pan_v4') }}">
                    {{ csrf_field() }}
        ...
        <div class="card card-success">
            ...
            <div class="card-body">
                ...
                @if(!empty($uanData['employment_history']))
                    <hr>
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @foreach($uanData['employment_history'] as $job)
                                <tr>
                                    <td>{{ $job['establishment_name'] ?? 'N/A' }}</td>
                                    <td>{{ $job['member_id'] ?? 'N/A' }}</td>
                                    <td>{{ $job['date_of_joining'] ?? 'N/A' }}</td>
                                    <td>{{ $job['date_of_exit'] ?? 'N/A' }}</td>
                                    <td>{{ $job['employment_period_in_months'] ?? 'N/A' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
