...
class EsignController extends Controller
{
            ...
            case 'one':
                ...
                if ($page_method_option == 'single') {
                    ...
                    if (count($existing_pdf) == 1) {
                        //second signature
                        $getEsignoutput = shell_exec('java -jar "/var/www/html/regtechapi/public/esign_jar/eSign2.1.jar" 1 "" "' . $sample_pdf_path . '" "ASPZTPLUAT007191" 1 "' . $url . '" "' . $certificate . '" "123456" "' . $tick_image . '" 15 "imported certificate" ' . $request->signature_stamp_page . ' "' . $request->name_show_signature . '" "' . $request->location_show_signature . '" "' . $request->reasone_for_signature . '" 205 0 120 60 "" "" 2>&1');
                        ...
                        $getEsignoutput = shell_exec('java -jar "C:\eSign2.1.jar" 1 "" "' . $sample_pdf_path . '" "BSPZTPLUAT007191" 1 "' . $url . '" "' . $certificate . '" "123456" "' . $tick_image . '" 15 "imported certificate" ' . $request->signature_stamp_page . ' "' . $request->name_show_signature . '" "' . $request->location_show_signature . '" "' . $request->reasone_for_signature . '" 25 0 120 60 "" "" 2>&1');
                ...
                $url = url("reponse_new/{$id}/{$page_method_option}");
                //first Signature
                $getEsignoutput = shell_exec('java -jar "/var/www/html/regtechapi/public/esign_jar/Multi_eSign2.1.jar" 1 "" "' . $sample_pdf_path . '" "ASPZTPLUAT007191" 1 "' . $url . '" "' . $certificate . '" "123456" "' . $tick_image . '" 15 "imported certificate" "' . $request->name_show_signature . '" "' . $request->location_show_signature . '" "' . $request->reasone_for_signature . '" "" "" "' . $pdfCordinateWidthAndHeight_Path . '" 2>&1');
    ...
    public function GenerateEsignXml2(Request $request)
    {
        // return 'xml2';
        // return $request->all();
            ...
            case 'one':
                ...
                if ($page_method_option == 'single') {
                    ...
                    if (count($existing_pdf) == 1) {
                        ...
                        $request_xml = file_get_contents($sample_pdf_request_xmlo);
                        // $html = view('e_sign_new.getE_signRequestXml', compact('getEsignoutput', 'request_xml'))->render();
                        return response()->json(['xml' => $request_xml]);
                    } elseif (count($existing_pdf) == 2) {
                        // return "count 2";
                        //third signature
                        $getEsignoutput = shell_exec('java -jar "/var/www/html/regtechapi/public/esign_jar/eSign2.1.jar" 1 "" "' . $sample_pdf_path . '" "ASPZTPLUAT007191" 1 "' . $url . '" "' . $certificate . '" "123456" "' . $tick_image . '" 15 "imported certificate" ' . $request->signature_stamp_page . ' "' . $request->name_show_signature . '" "' . $request->location_show_signature . '" "' . $request->reasone_for_signature . '" 355 0 120 60 "" "" 2>&1');
                        ...
                        //first Signature
                        //$getEsignoutput = shell_exec('"C:\Program Files\Java\jdk-1.8\bin\java.exe" -jar "C:\eSign2.1.jar" 1 "" "' . $sample_pdf_path . '" "ASPZTPLUAT007191" 1 "' . $url . '" "' . $certificate . '" "123456" "' . $tick_image . '" 15 "imported certificate" ' . $request->signature_stamp_page . ' "' . $request->name_show_signature . '" "' . $request->location_show_signature . '" "' . $request->reasone_for_signature . '" 55 0 120 60 "" "" 2>&1');
                        $getEsignoutput = shell_exec('java -jar "/var/www/html/regtechapi/public/esign_jar/eSign2.1.jar" 1 "" "' . $sample_pdf_path . '" "ASPZTPLUAT007191" 1 "' . $url . '" "' . $certificate . '" "123456" "' . $tick_image . '" 15 "imported certificate" ' . $request->signature_stamp_page . ' "' . $request->name_show_signature . '" "' . $request->location_show_signature . '" "' . $request->reasone_for_signature . '" 25 0 120 60 "" "" 2>&1');
    ...
    //     $apiUrl = 'http://localhost:8000/api/getSignDocument';
    ...
    //     fclose($myfile);
    //     // return redirect()->route('getsign.document', ['id' => $id, 'sign_method' => $sign_method]);
    //     $response = Http::get(url("api/getsign/{$id}/{$sign_method}"));

        
    //         // Get the response data (assume it is in JSON format)
    //         $responseData = $response->json(); // This is the data you want to pass in the query string
            
    //         // Prepare the redirect URL with the response data
    //         $redirectUrl = "http://localhost:5173/response_new?params={$responseData}";
    
    //         // Redirect to the new URL with response data as a query parameter
    ...
    {    
        $output = $request->msg;
        $myfile = fopen('/var/www/html/regtechapi/public/esign_new/response.txt', 'w');
        fwrite($myfile, $output);
        fclose($myfile);
        return redirect()->route('getsign.document', ['id' => $id, 'sign_method' => $sign_method]);
    }
...
{
    // Get the XML response from the incoming request
    $request_xml = $request->input('xmlResponse'); // This is the `msg` you're sending from the frontend
    
    // Return the view and pass the `xmlResponse` to the view
    return view('e_sign_new.getE_signRequestXml', compact('request_xml'));
}


public function getSignDocument(Request $request)
    {
        // return "http://127.0.0.1:8000/esign_new_pdf/New_Aug_Sept_Bill_with_sign_signedFinal.pdf";
        ...
        if ($sign_method == 'single') {
            ...
            // return $pdf_path;
            $response_path = '/var/www/html/regtechapi/public/esign_new/response.txt';
            $tick_image = public_path('esign_new\tick_imge.jpeg');
            ...
            if (count($existing_pdf) == 1) {
                ...
                $jarPath = "/var/www/html/regtechapi/public/esign_jar/eSign2.1.jar";
                //second signature
        ...
        $javaPath = "/usr/bin/java";  
        $jarPath = "/var/www/html/regtechapi/public/esign_jar/Multi_eSign2.1.jar";
        $esign_info = EsignInfo::where('id', $id)->first();
        $name_of_signature = $esign_info->name_of_signature;
        $location_of_signature = $esign_info->location_of_signature;
        $reasone_for_signature = $esign_info->reasone_for_signature;
    ...
    public function getSignDocument2(Request $request)
    {
        // return "http://127.0.0.1:8000/esign_new_pdf/New_Aug_Sept_Bill_with_sign_signedFinal.pdf";
        // return "authorization failed";
        $id = $request->id;
        ...
        if ($sign_method == 'single') {
            ...
            if (count($existing_pdf) == 1) {
                ...
                if (file_exists($final_signed_pdf_path)) {
                    ...
                    // return redirect()->away("http://localhost:5173/response_new?params={$download_pdf}&id={$id}&sign_method={$sign_method}");
                    // return Response::download($final_signed_pdf_path);
                    return Response::stream(function() use ($final_signed_pdf_path) {
                        $file = fopen($final_signed_pdf_path, 'r');
                        while (!feof($file)) {
                            echo fread($file, 1024); // Read in chunks
                        }
                        fclose($file);
                    }, 200, [
                        'Content-Type' => 'application/pdf',
                        'Content-Disposition' => 'attachment; filename="signed_document.pdf"',
                        'Content-Length' => filesize($final_signed_pdf_path)
                    ]);
                    // return view('e_sign_new.getFinalSignedDocument', compact('download_pdf'));
                }
                // return redirect()->away("http://localhost:5173/response_new?params=e&id={$id}&sign_method={$sign_method}");
        ...
        $response_path = '/var/www/html/regtechapi/public/esign_new/response.txt';
        $tick_image = public_path('esign_new\tick_imge.jpeg');
        $pdfCordinateWidthAndHeight_Path = '/var/www/html/regtechapi/public/esign_new/list.txt';
        $getEsignDocumentOutput = shell_exec('"'.$javaPath.'" -jar "'.$jarPath.'" 2 "' . $response_path . '" "' . $pdf_path . '" "' . $tick_image . '" 15 "' . $name_of_signature . '" "' . $location_of_signature . '" "' . $reasone_for_signature . '" "" "" "' . $pdfCordinateWidthAndHeight_Path . '" 2>&1');
        $signed_pdf_filename = pathinfo($pdf_file, PATHINFO_FILENAME);
        $final_signed_pdf_filename = $signed_pdf_filename . '_signedFinal' . '.' . 'pdf';
        $signature_pdf_file = new EsignSignaturePdf();
...
//         if (count($existing_pdf) == 1 || count($existing_pdf) == 2 || count($existing_pdf) == 3) {
//             // Handle second, third or other signature cases
//             $signature_positions = [1 => 15, 2 => 355, 3 => 505];  // Define signature positions based on count
//             $position = $signature_positions[count($existing_pdf)] ?? 55; // Default to 55 if not found

//             $getEsignDocumentOutput = shell_exec('"C:\Program Files\Java\jdk-1.8\bin\java.exe" -jar "C:\eSign2.1.jar" 2 "' . $response_path . '" "' . $pdf_path . '" "' . $tick_image . '" 15 ' . $signature_stamp_insert_location . ' "' . $name_of_signature . '" "' . $location_of_signature . '" "' . $reasone_for_signature . '" ' . $position . ' 0 120 60 "" "" -o "output.pdf" 2>&1');

//             // Final signed PDF filename
//             $signed_pdf_filename = pathinfo($pdf_file, PATHINFO_FILENAME);
//             $final_signed_pdf_filename = $signed_pdf_filename . '_signedFinal' . '.' . 'pdf';
...
//             // File path for the signed PDF
//             $final_signed_pdf_path = public_path('esign_new_pdf') . '\\' . $final_signed_pdf_filename;

//             // Check if file exists, then redirect with success or error
//             if (file_exists($final_signed_pdf_path)) {
//                 $download_pdf = asset('esign_new_pdf/' . $final_signed_pdf_filename);
//                 return redirect()->away("http://localhost:5173/response_new?params={$download_pdf}&id={$id}&sign_method={$sign_method}");
//             }
...
//         } else {
//             // Handle the case for no existing PDFs or more than 3 signatures
//             $getEsignDocumentOutput = shell_exec('"C:\Program Files\Java\jdk-1.8\bin\java.exe" -jar "C:\Multi_eSign2.1.jar" 2 "' . $response_path . '" "' . $pdf_path . '" "' . $tick_image . '" 15 "' . $name_of_signature . '" "' . $location_of_signature . '" "' . $reasone_for_signature . '" "" "" "' . public_path('esign_new\list.txt') . '" 2>&1');
...
//     // For cases where the sign_method is not 'single'
//     $esign_info = EsignInfo::where('id', $id)->first();
//     $name_of_signature = $esign_info->name_of_signature;
//     $location_of_signature = $esign_info->location_of_signature;
//     $reasone_for_signature = $esign_info->reasone_for_signature;
