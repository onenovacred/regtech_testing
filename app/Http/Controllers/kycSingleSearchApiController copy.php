    ...
    public function searchApi(Request $request){
        ...
        if($request->isMethod('POST')){
          ...
          elseif(!empty($request->apies) && $request->get('apies')=="PancardOcr"){
              if (Auth::user()->scheme_type != 'demo') {
                  ...
                  CURLOPT_POSTFIELDS => ['file' => new \CURLFILE($_FILES['panocrfile']['tmp_name'], $_FILES['panocrfile']['type'], $_FILES['panocrfile']['name']),
                                          'file_type'=>'pancard'],
              ...
              if (Auth::user()->scheme_type != 'demo') {
                  if (Auth::user()->role_id == 1) {
                      ...
                      $accessToken = Auth::user()->access_token;
                       ...
                       CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                       CURLOPT_CUSTOMREQUEST => 'POST',
                       CURLOPT_POSTFIELDS => ['file' => new \CURLFILE($_FILES['voteridocrfile']['tmp_name'], $_FILES['voteridocrfile']['type'], $_FILES['voteridocrfile']['name']),
                                              'file_type'=>'voterid'],
                       CURLOPT_HTTPHEADER => ["AccessToken:$accessToken"],
                    ]);
                       $response = curl_exec($curl);
          ...
          elseif(!empty($request->apies) && $request->get('apies')=="PassportOcr"){
              if (Auth::user()->scheme_type != 'demo') {
                  ...
                  curl_setopt_array($curl, [
                      ...
                      CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                      CURLOPT_CUSTOMREQUEST => 'POST',
                      CURLOPT_POSTFIELDS => ['file' => new \CURLFILE($_FILES['passportocrfile']['tmp_name'], $_FILES['passportocrfile']['type'], $_FILES['passportocrfile']['name']),
                      'file_type'=>'passport'],
                      CURLOPT_HTTPHEADER => ['AccessToken:'.$accessToken],
                  ]);
              ...
              if (Auth::user()->scheme_type != 'demo') {
                  if (Auth::user()->role_id == 1) {
                    ...
                    curl_setopt_array($curl, [
                        ...
                        CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                        CURLOPT_CUSTOMREQUEST => 'POST',
                        CURLOPT_POSTFIELDS =>['file' => new \CURLFILE($_FILES['aadharmaskocrfile']['tmp_name'], $_FILES['aadharmaskocrfile']['type'], $_FILES['aadharmaskocrfile']['name']),
               ...
               if (Auth::user()->role_id == 1) {
                 ...
                 $api_id = $apiamster->id;
                  ...
                  curl_setopt_array($curl, [
                      ...
                      CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                      CURLOPT_CUSTOMREQUEST => 'POST',
                      CURLOPT_POSTFIELDS =>['file' => new \CURLFILE($_FILES['aadharocrfile']['tmp_name'], $_FILES['aadharocrfile']['type'], $_FILES['aadharocrfile']['name']),
          ...
          elseif(!empty($request->apies) && $request->get('apies')=="DrivingLicenseOcr"){
              if (Auth::user()->scheme_type != 'demo') {
                  ...
                  curl_setopt_array($curl, [
                      ...
                      CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                      CURLOPT_CUSTOMREQUEST => 'POST',
                      CURLOPT_POSTFIELDS =>['file' => new \CURLFILE($_FILES['drivingocrfile']['tmp_name'], $_FILES['drivingocrfile']['type'], $_FILES['drivingocrfile']['name']),
        ...
        if(Auth::user()->scheme_type!='demo') {
          ...
          elseif(!empty($request->apies) && $request->get('apies')=="PassportVerification"){
            ...
            $json = ['id_number'=> $request->passport_file_number,
        ...
        elseif(!empty($request->apies) && $request->get('apies')=="FaceDetection"){
            if (Auth::user()->scheme_type != 'demo') {
                ...
                curl_setopt_array($curl, array(
                 ...
                 CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                 CURLOPT_CUSTOMREQUEST => 'POST',
                 CURLOPT_POSTFIELDS =>['face_image' => new \CURLFILE($_FILES['fdimage_file']['tmp_name'], $_FILES['fdimage_file']['type'], $_FILES['fdimage_file']['name']),
                 ],
                 CURLOPT_HTTPHEADER => array(
                   'AccessToken:'.$accessToken,
                 ),
        ...
        elseif(!empty($request->apies) && $request->get('apies')=="DetectedEmotion"){
            ...
            if (Auth::user()->scheme_type != 'demo') {
                ...
                curl_setopt_array($curl, array(
                 ...
                 CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                 CURLOPT_CUSTOMREQUEST => 'POST',
                 CURLOPT_POSTFIELDS =>['image_file' => new \CURLFILE($_FILES['edimage_file']['tmp_name'], $_FILES['edimage_file']['type'], $_FILES['edimage_file']['name']),
                 ],
                 CURLOPT_HTTPHEADER => array(
                   'AccessToken:'.$accessToken,
                 ),
