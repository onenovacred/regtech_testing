...
class KycController extends Controller
{
      ...
      public function getPlanValues(Request $request)
    {
        ...
        if ($request->plan == 'basic') {
            $plan_details = DB::table('api_master')
                ->select('id', 'basic_price as price', 'api_group_id')
                ->get();
        } elseif ($request->plan == 'starter') {
            $plan_details = DB::table('api_master')
                ->select('id', 'starter_price as price', 'api_group_id')
                ->get();
        } elseif ($request->plan == 'standard') {
            $plan_details = DB::table('api_master')
                ->select('id', 'standard_price as price', 'api_group_id')
                ->get();
        } elseif ($request->plan == 'advance') {
            $plan_details = DB::table('api_master')
                ->select('id', 'advance_price as price', 'api_group_id')
                ->get();
        } elseif ($request->plan == 'growth') {
            $plan_details = DB::table('api_master')
                ->select('id', 'growth_price as price', 'api_group_id')
                ->get();
        } elseif ($request->plan == 'unicorn') {
            $plan_details = DB::table('api_master')
                ->select('id', 'unicorn_price as price', 'api_group_id')
                ->get();
        }
    ...
    public function aadhaar_upload(Request $request){
        ...
        if($request->isMethod('POST')){
            ...
            $aadhaarOCR = json_decode($result, true);
              //dd($aadhaarOCR);
            $statusCode = $aadhaarOCR['status_code'];
            $aadhaar_number = null;
    ...
    public function voter_upload(Request $request){
        ...
        if($request->isMethod('POST')){
            ...
            $voter = $voterocr[0]['ocr_fields'];
            $voter_detailed_info = $voterocr[0]['voter_validation'];
            $statusCode = $voterocr[0]['statusCode'];
        ...
        if ($request->isMethod('POST')) {
            ...
            if (Auth::user()->scheme_type != 'demo') {
                ...
                try {
                    ...
                    $statusCode = $e->getResponse()->getStatusCode();
                    if ($pancardError['statusCode'] == 102) {
                        $statusCode = 102;
                        $errorMessage = 'PAN Number InValid Please Enter Correct PanNumber.';
        ...
        if($request->isMethod('POST')){
            ...
            if(Auth::user()->scheme_type!='demo') {
                ...
                try{
                    ...
                    $statusCode = $e->getResponse()->getStatusCode();
                    $errorMessage = 'Error. Please contact techsupport@docboyz.in. for more details';
                    return view('esign.esign_docboyz', compact('statusCode','errorMessage'));
                }
            } else {
                $scheme_type = SchemeTypeMaster::where('id',Auth::user()->scheme_type_id)->first();
                $hit_count_remaining = $scheme_type->hit_limits - Auth::user()->scheme_hit_count;
